...
const PersonalInfoSection = ({ form, handleChange }) => {
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = () => {
      handleChange({ target: { name: "profilePicture", value: reader.result } });
    };
      ...
      <div className="px-6 py-5 flex items-start gap-6">
        ...
        <div className="flex flex-col items-center gap-2 shrink-0">
          <button
            ...
            className="w-20 h-20 rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50 flex items-center justify-center hover:border-blue-400 hover:bg-blue-100 transition-all duration-200 overflow-hidden"
          >
            {form.profilePicture ? (
        ...
        <div className="flex-1">
          ...
          <textarea
            ...
            className="w-full text-sm text-slate-800 bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 outline-none placeholder:text-slate-300 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 resize-none transition-all duration-150"
          />
        </div>
      </div>
    </div>
