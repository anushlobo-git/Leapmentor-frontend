...
const IntegrationsSection = ({ googleCalendarConnected, onConnectionChange }) => {
  ...
  // ── Connect: open Google OAuth in a popup ──────────────────
      ...
      <div className="flex items-center justify-between gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <span
                className={`w-1.5 h-1.5 rounded-full ${
                  googleCalendarConnected ? "bg-green-500" : "bg-red-400"
                }`}
              />
              <span
                className={`text-xs font-medium ${
                  googleCalendarConnected ? "text-green-600" : "text-red-500"
                }`}
              >
        ...
        {googleCalendarConnected ? (
          <button
            ...
            disabled={loading}
            className="text-xs font-semibold px-4 py-2 rounded-xl border-2 border-red-200 text-red-500 bg-white hover:bg-red-50 hover:border-red-400 transition-all duration-150 shrink-0 disabled:opacity-50"
          >
            {loading ? "..." : "Disconnect"}
          </button>
        ) : (
          <button
            ...
            disabled={loading}
            className="text-xs font-semibold px-4 py-2 rounded-xl border-2 border-blue-200 text-blue-600 bg-white hover:bg-blue-50 hover:border-blue-400 transition-all duration-150 shrink-0 disabled:opacity-50"
          >
