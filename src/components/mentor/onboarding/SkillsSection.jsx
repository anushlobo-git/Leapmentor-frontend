    ...
    <div className="bg-white rounded-2xl border border-[#e8edf5] shadow-sm overflow-hidden">
      ...
      <div className="px-6 py-5">
        <label className="block text-xs font-semibold text-[#475569] mb-2">
          Core Skills <span className="text-[#2563eb]">*</span>
        </label>
        ...
        <div className="flex flex-wrap gap-2 mb-3 min-h-[32px]">
          {(form.skills || []).map((skill) => (
            <span
              key={skill}
              className="inline-flex items-center gap-1.5 bg-[#dbeafe] text-[#1d4ed8] text-xs font-semibold px-3 py-1.5 rounded-full"
            >
              {skill}
              <button
                type="button"
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={addSkill}
            className="px-4 py-2.5 rounded-xl bg-[#2563eb] text-white text-xs font-bold hover:bg-[#1d4ed8] transition-colors duration-150"
          >
            Add
          </button>
        </div>
