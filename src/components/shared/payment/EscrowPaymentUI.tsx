...
interface EscrowModalShellProps {
  title: string;
  onClose: () => void;
  onPay: () => void;
  payDisabled: boolean;
...
export const SessionDetailRows = ({ rows }: { rows: (SessionDetailRow | false | null | undefined)[] }) => (
  <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
    ...
    <div className="space-y-1.5">
      {rows.filter(Boolean).map((row) => {
        const { label, value } = row as SessionDetailRow;
        return (
...
export const BalanceRow = ({ fetching, walletBalance, insufficient }: BalanceRowProps) => (
