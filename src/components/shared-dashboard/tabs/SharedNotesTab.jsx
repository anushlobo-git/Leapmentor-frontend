      ...
      >
        {selectedFile ? (
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm font-semibold text-slate-600">Drop file here or <span className="text-blue-600">click to browse</span></p>
            <p className="text-xs text-slate-400">PDF, Word, PowerPoint, Excel, Images, Text · Max 10MB</p>
          </div>
        )}
...
const NoteCard = ({ note, myId, onDelete, isPrivateView = false }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 flex gap-3 hover:border-slate-300 hover:shadow-sm transition-all duration-150">
    ...
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
        <input
          ...
          className="flex-1 text-sm font-bold text-slate-800 bg-transparent outline-none placeholder:text-slate-400"
        />
        <div className="flex items-center gap-2 ml-3">
          {/* Download .txt */}
          <button type="button" onClick={downloadTxt}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-500 text-xs font-semibold hover:bg-slate-100 transition-all">
          ...
          <button type="button" onClick={handleSave} disabled={saving || !dirty}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
            {saving ? "Saving..." : "Save"}
          </button>
          {/* Delete */}
          {note?._id && (
      ...
      <textarea
        ...
        className="w-full h-64 px-4 py-3 text-sm text-slate-700 leading-relaxed resize-none outline-none bg-white placeholder:text-slate-300"
      />
      {dirty && (
...
const FilesSection = ({ connect, isPrivateView }) => {
  ...
  const handleUpload = async (file, title) => {
    return await uploadNote(file, title, isPrivateView);
  };
  const handleDelete = async (noteId) => {
    return await deleteNote(noteId, isPrivateView);
  };
...
const SharedNotesTab = ({ connect }) => {
  const [activeView,       setActiveView]       = useState("shared");   // shared | private
  const [privateSubTab,    setPrivateSubTab]    = useState("files");    // files | notepad
    ...
    <div className="space-y-0">
      ...
      <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1 mb-5 w-fit">
        <button onClick={() => setActiveView("shared")}
          className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeView === "shared" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          🌐 Shared
        </button>
        <button onClick={() => setActiveView("private")}
          className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeView === "private" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          🔒 Private
        </button>
      </div>
      ...
      {/* Content */}
      {!isPrivateView && (
        <FilesSection connect={connect} isPrivateView={false} />
      )}
      {isPrivateView && privateSubTab === "files" && (
        <FilesSection connect={connect} isPrivateView={true} />
      )}
      {isPrivateView && privateSubTab === "notepad" && (
