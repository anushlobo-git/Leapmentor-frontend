...
import {
  ...
  formatDate,
  getInitials,
} from "@features/mentee/views/dashboard/history/constants";
import StatusBadge from "@features/mentee/views/dashboard/history/StatusBadge";
import EscrowPaymentModal from "@features/mentee/views/dashboard/history/EscrowPaymentModal";
import {
  PendingContent,
  AcceptedContent,
  CompletedContent,
  RejectedContent,
} from "@features/mentee/views/dashboard/history/RequestStatusViews";
import {
  OngoingContent,
  ReferredContent,
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-14 bottom-0 w-80 z-50 bg-white border-l border-slate-100 shadow-2xl flex flex-col">
        ...
        {/* Status banner */}
        <div
          className={`mx-5 mt-4 rounded-xl px-3 py-2.5 text-xs font-medium ${STATUS_STYLES[status as keyof typeof STATUS_STYLES]}`}
        >
