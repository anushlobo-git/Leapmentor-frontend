...
import useMentorProfileModalPresenter, {
  ...
  type SlotObj,
  type SlotGroup,
} from "@features/mentee/presenters/useMentorProfileModalPresenter";

const formatTime = formatTimeOfDay;
const StarRating = ({ rating, reviewCount }: { rating?: number | string; reviewCount?: number }) => {
...
// ── Slot Pill ─────────────────────────────────────────────────
const SlotPill = ({ slot, group, selected, maxReached, onToggle }: { slot: SlotObj; group: SlotGroup; selected: boolean; maxReached: boolean; onToggle: (slot: SlotObj, group: SlotGroup) => void }) => {
...
// ── Selected Slot Row ─────────────────────────────────────────
const SelectedSlotRow = ({ slot, index, onRemove }: { slot: SlotObj; index: number; onRemove: (index: number) => void }) => (
...
// ── Main Modal ────────────────────────────────────────────────
const MentorProfileModal = ({ mentor, onClose }: { mentor: MentorProfile; onClose: () => void }) => {
  const {
    MAX_SLOTS,
    user,
    currentRole,
    ...
    showSuccess,
  } = useMentorProfileModalPresenter(mentor, onClose);
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedSlots.length > 0 && (
                    <div className="flex items-center gap-1 bg-blue-900 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                      {Array.from({ length: MAX_SLOTS }).map((_, i: number) => (
                        <span
                          key={`selection-dot-${i + 1}`}
                ...
                {!fetchingSlots &&
                  ...
                  availableGroups.length > 0 && (
                    <>
                      {/* Day tabs */}
                      <div className="flex gap-2">
                        {availableGroups.map((group: SlotGroup, idx: number) => {
