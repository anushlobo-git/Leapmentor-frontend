...
const TagInput = ({ tags, onAdd, onRemove, placeholder, error, inputId }: TagInputProps) => {
 ...
 */
interface InterestedFieldsForm {
  interestedFields?: string[];
  skills?: string[];
}

interface InterestedFieldsErrors {
  interestedFields?: boolean;
  skills?: boolean;
}

interface InterestedFieldsSectionProps {
  form: InterestedFieldsForm;
  handleChange: (event: { target: { name: string; value: string[] } }) => void;
  errors?: InterestedFieldsErrors;
}

const InterestedFieldsSection = forwardRef<HTMLDivElement, InterestedFieldsSectionProps>(
  ({ form, handleChange, errors = {} }, ref) => {
    const addToArray = (field: keyof InterestedFieldsForm, value: string) => {
      handleChange({
        target: { name: field, value: [...(form[field] || []), value] },
      });
    };

    const removeFromArray = (field: keyof InterestedFieldsForm, value: string) => {
      handleChange({
        target: {
          name: field,
