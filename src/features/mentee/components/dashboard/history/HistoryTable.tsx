...
import { formatDate, getInitials } from "@features/mentee/components/dashboard/history/constants";
...
interface HistoryTableProps {
  ...
  selected: HistoryRequest | null;
  onSelect: (request: HistoryRequest | null) => void;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-[2.5fr_1.5fr_1.2fr_1fr_1fr] gap-4 px-5 py-2.5 bg-slate-50 rounded-xl border border-slate-100">
        {["Mentor", "Skill / Role", "Date Sent", "Status", "Actions"].map((col: string) => (
          <p key={col} className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">{col}</p>
        ))}
      </div>

      {/* Cards */}
      {requests.map((request: HistoryRequest) => {
