...
import {
  sendAiChatMessage,
  sendSupportMessage,
} from "@features/support/models/support.api";
...
function buildSystemPrompt(role: string, userContext: UserContext) {
...
export default function LeapBuddy({
  ...
  const [ticketForms, setTicketForms] = useState<Record<number, TicketForm>>({});
  ...
  const submitTicket = async (idx: number) => {
