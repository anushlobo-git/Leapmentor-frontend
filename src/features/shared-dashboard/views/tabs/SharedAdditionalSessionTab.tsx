...
import { getMentorAvailabilityForConnect } from "@features/sessions/models/sessions.api";
...
interface ScheduledSessionSlot extends SessionSlot {
  date: string;
  day: string;
}
...
interface SlotPillProps {
  ...
  onToggle: (slot: SessionSlot, group: AvailabilityGroup) => void;
  isBooked: boolean;
}

interface ConfirmModalProps {
  slot: ScheduledSessionSlot;
  onConfirm: () => void;
  onCancel: () => void;
  saving?: boolean;
}
...
interface AdditionalSessionPaymentModalProps {
  slot: ScheduledSessionSlot;
  slotId: string;
  onClose: () => void;
  onSuccess: () => void;
...
interface LegacyAdditionalSessionsState extends ReturnType<typeof useSessions> {
  additionalSlots?: SessionSlot[];
  saving?: boolean;
}
...
// ── Slot Pill ──────────────────────────────────────────────────
const SlotPill = ({ slot, group, onToggle, isBooked }: SlotPillProps) => {
  return (
    <button
      type="button"
...
// ── Confirm Modal ─────────────────────────────────────────────
const ConfirmModal = ({ slot, onConfirm, onCancel, saving }: ConfirmModalProps) => {
...
// ── Success Screen ────────────────────────────────────────────
const SuccessScreen = ({ slot, onDone }: SuccessScreenProps) => {
  const dateLabel = new Date(slot.date + "T00:00:00").toLocaleDateString("en-US", {
    weekday: "long", month: "long", day: "numeric", year: "numeric",
  });
  return (
...
const TokenIcon = ({ size = 13 }: { size?: number }) => (
...
const AdditionalSessionPaymentModal = ({ slot, slotId, onClose, onSuccess }: AdditionalSessionPaymentModalProps) => {
  ...
  const paymentShortfall = walletBalance === null ? 0 : totalAmount - walletBalance;
  ...
  const handlePay = async () => {
    ...
    if (insufficient) { setError(`Need ${paymentShortfall} more tokens.`); return; }
    try {
      setLoading(true);
          ...
          <div className="overflow-y-auto flex-1 px-4 py-3 space-y-3">
            ...
            {insufficient && (
              <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
                ...
                <p className="text-xs text-red-600 font-medium">You need {paymentShortfall} more tokens.</p>
              </div>
            )}
...
const SharedAdditionalSessionTab = () => {
  ...
  const connectId = useSelector((state: RootState) => selectConnectId(state));
  const connect = useSelector((state: RootState) => selectConnect(state));
  const isCompleted = useSelector((state: RootState) => selectConnectStatus(state)) === "completed";
  const isMentor = useSelector((state: RootState) => selectViewerRole(state)) === "mentor";
  const mentorName = connect?.mentor?.name || "Mentor";
  ...
  const [confirmSlot, setConfirmSlot] = useState<ScheduledSessionSlot | null>(null);
  const [successSlot, setSuccessSlot] = useState<ScheduledSessionSlot | null>(null);
  const [paymentSlot, setPaymentSlot] = useState<{ slot: ScheduledSessionSlot; slotId: string } | null>(null);

  const { slots: untypedSlots, additionalSlots, saving, addSlot } = useSessions(connectId, undefined) as LegacyAdditionalSessionsState;
  const slots = untypedSlots as SessionSlot[];

  const existingSlotDates = [...slots, ...(additionalSlots || [])].map((s) => ({
    date: s.date, startTime: s.startTime, endTime: s.endTime,
  }));

  const fetchAvailability = async (dur: number) => {
    if (!connectId) return;
    try {
      setAvailLoading(true);
      setAvailError("");
      const res = await getMentorAvailabilityForConnect(connectId, dur);
  ...
  const toggleSlot = (slot: SessionSlot, group: AvailabilityGroup) => {
    const slotObj = {
      ...slot,
      date: group.date,
      day: group.day,
