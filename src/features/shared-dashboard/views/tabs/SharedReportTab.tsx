...
import ReportModal from "@features/shared-dashboard/views/tabs/ReportModal";
import ReportSuccessModal from "@features/shared-dashboard/views/tabs/ReportSuccessModal";
...
interface ReportState {
  ...
  submitting: boolean;
  error: string | null;
  submitFeedback: (rating: number, comment: string) => Promise<{ success?: boolean }>;
}

interface StarRatingInputProps { value: number; onChange: (rating: number) => void; disabled?: boolean; }
interface FeedbackCardProps { feedback: Feedback; label: string; isOwn: boolean; }
interface SharedReportTabProps { connect?: SharedReportConnect; reportRefreshKey?: string | number; }
...
// ── Feedback Card ─────────────────────────────────────────────
const FeedbackCard = ({ feedback, label, isOwn }: FeedbackCardProps) => (
  <div
...
const NotCompletedState = () => (
  <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
    <div
      className="w-14 h-14 rounded-2xl bg-amber-50 border border-amber-100
    ...
    <div>
      ...
      <p className="text-xs text-slate-400 mt-1 max-w-xs leading-relaxed">
        Feedback can only be submitted after both parties mark all sessions as
        complete.
      </p>
    </div>
  </div>
...
// ── Main ──────────────────────────────────────────────────────
const SharedReportTab = ({ connect, reportRefreshKey }: SharedReportTabProps) => {
  ...
  const {
    ...
    error,
    submitFeedback,
  } = useReport(connect?._id, reportRefreshKey as number) as unknown as ReportState; // 👈 ADDED: pass reportRefreshKey
