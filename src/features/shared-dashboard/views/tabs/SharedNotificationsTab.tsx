...
interface NotifCardProps {
  ...
  onMarkRead: (id: NotificationItem["id"]) => Promise<void>;
  onDelete: (id: NotificationItem["id"]) => Promise<void>;
  setActiveTab?: (tab: string) => void;
  role?: ViewerRole;
}

interface SharedNotificationsTabProps {
  setActiveTab: (tab: string) => void;
  role?: ViewerRole;
}
...
const getAvatarColor = (id = "") => {
  const sum = id.split("").reduce((acc, c) => acc + (c.codePointAt(0) ?? 0), 0);
...
const resolveNavigation = (notif: NotificationItem, setActiveTab?: (tab: string) => void, role?: ViewerRole) => {
...
// ── Notification Card ─────────────────────────────────────────
const NotifCard = ({ notif, onMarkRead, onDelete, setActiveTab, role }: NotifCardProps) => {
  const cfg = TYPE_CONFIG[notif.type] || TYPE_CONFIG["new_message"];
  const initials = getInitials(notif.senderName || notif.title);
  const avatarBg = getAvatarColor(String(notif.id));
  ...
  return (
    <div
      className={`relative rounded-2xl border px-3.5 py-3.5 sm:px-5 sm:py-4 transition-all duration-200 hover:shadow-md group
    ...
    >
      {!notif.read && (
        <div className="absolute left-0 top-5 bottom-5 w-0.75 rounded-r-full bg-blue-500" />
      )}
      ...
      <div className="flex items-start gap-3 sm:gap-4">
        <button
          ...
          className="flex-1 min-w-0 flex items-start gap-3 sm:gap-4 text-left rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-200"
        >
      ...
      {notif.actions?.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3 ml-12 sm:ml-15">
