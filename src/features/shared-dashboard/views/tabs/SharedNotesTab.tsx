...
import useNotes from "@features/notes/presenters/useNotes";
import PrivateNotesTab from "@features/shared-dashboard/views/tabs/PrivateNotesTab";
import {
  selectConnectId,
  selectConnectStatus,
...
import {
  formatFileSize,
  FILE_TYPE_CONFIG,
} from "@features/notes/presenters/notesHelpers";
import {
  groupNotesByDay,
  downloadNoteFile,
  NoteDateSeparator,
...
} from "@features/shared-dashboard/views/tabs/notesTabShared";
...
interface NotesState {
  ...
  uploading: boolean;
  error: string | null;
  uploadNote: (file: File, title: string, isPrivate: boolean) => Promise<{ success?: boolean } | undefined>;
  deleteNote: (noteId: string, isPrivate: boolean) => Promise<{ success?: boolean } | undefined>;
}

interface UploadModalProps {
  onUpload: (file: File, title: string) => Promise<{ success?: boolean } | undefined>;
  uploading: boolean;
  onClose: () => void;
...
// ── Upload Modal ──────────────────────────────────────────────
const UploadModal = ({ onUpload, uploading, onClose, isPrivateView }: UploadModalProps) => {
  ...
  const handleFileInput = (e: ChangeEvent<HTMLInputElement>) => validateAndSet(e.target.files?.[0]);
...
const NoteCard = ({ note, myId, onDelete, isPrivateView = false }: NoteCardProps) => {
  const [deleting, setDeleting] = useState(false);
  const uploaderId = typeof note.uploadedBy === "string"
    ? note.uploadedBy
    : note.uploadedBy?._id;
  const uploaderName = typeof note.uploadedBy === "string"
    ? undefined
    : note.uploadedBy?.name;
  const isOwn = uploaderId === myId;
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="flex items-start justify-between gap-2">
          ...
          {!isPrivateView && (
            ...
            >
              {isOwn ? "You" : uploaderName || "Partner"}
            </span>
          )}
          {isPrivateView && (
...
const SharedFilesSection = ({ myId }: { myId?: string }) => {
  ...
  const { notes, loading, uploading, error, uploadNote, deleteNote } =
    useNotes(connectId) as NotesState;

  const handleUpload = async (file, title) => uploadNote(file, title, false);
  const handleDelete = async (noteId) => deleteNote(noteId, false);

  const groupedItems = groupNotesByDay(notes) as Array<
    | { type: "separator"; key: string; dateStr?: string }
    | { type: "note"; key: string; note: SharedNote }
  >;
...
const SharedNotesTab = () => {
  ...
  const myId = useSelector(
    (state: RootState) => (state.auth.user as { id?: string } | null)?.id,
  );
