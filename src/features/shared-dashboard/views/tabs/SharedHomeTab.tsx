/**
 * Copyright (c) 2026 Leapmentor. All rights reserved.
 */

// src/features/shared-dashboard/components/tabs/SharedHomeTab.jsx
import { useState, useCallback } from "react";
import type { ReactNode } from "react";
import { useDispatch, useSelector } from "react-redux";
import type { AppDispatch } from "@store/index";
import { useSearchParams } from "react-router-dom";
import ReportModal from "@features/shared-dashboard/views/tabs/ReportModal";
import ReportSuccessModal from "@features/shared-dashboard/views/tabs/ReportSuccessModal";
import {
  selectConnect,
  setActiveTab,
} from "@features/shared-dashboard/models/sharedDashboardSlice";
import {
  formatFullSlot,
  formatDateString as formatDate,
} from "@lib/formatters/dateTime";

const getInitials = (name = "") =>
  name
    .split(" ")
    .map((w) => w[0])
    .join("")
    .toUpperCase()
    .slice(0, 2);

const GRADIENTS = [
  "linear-gradient(135deg, #3b82f6, #1d4ed8)",
  "linear-gradient(135deg, #8b5cf6, #6d28d9)",
  "linear-gradient(135deg, #10b981, #047857)",
  "linear-gradient(135deg, #f59e0b, #b45309)",
  "linear-gradient(135deg, #ef4444, #b91c1c)",
];

const getGradient = (name = "") =>
  GRADIENTS[name.codePointAt(0) % GRADIENTS.length];

const formatSlot = formatFullSlot;

interface IconBaseProps {
  size?: number;
  children: ReactNode;
}

interface IconProps {
  size?: number;
}

interface PersonProfile {
  profilePicture?: string;
  currentRole?: string;
  company?: string;
  skills?: string[];
}

interface PersonCardProps {
  name: string;
  profile?: PersonProfile | null;
  roleLabel: string;
}

interface InfoRowProps {
  icon: ReactNode;
  label: string;
  value?: ReactNode;
  accent?: string;
}

interface QuickActionProps {
  icon: ReactNode;
  label: string;
  onClick: () => void;
  color?: string;
}

interface SharedSessionSlot {
  date?: string;
  startTime?: string;
  endTime?: string;
  status?: string;
}

interface SharedHomeTabProps {
  slots?: SharedSessionSlot[];
}

// ── Icons ─────────────────────────────────────────────────────
// Shared SVG wrapper so each icon only needs to declare its own
// paths, instead of repeating the same svg attributes every time.
const IconBase = ({ size = 14, children }: IconBaseProps) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
    {children}
  </svg>
);

const CalendarIcon = (props: IconProps) => (
  <IconBase {...props}>
    <rect x="3" y="4" width="18" height="18" rx="2" />
    <line x1="16" y1="2" x2="16" y2="6" />
    <line x1="8" y1="2" x2="8" y2="6" />
    <line x1="3" y1="10" x2="21" y2="10" />
  </IconBase>
);

const TokenIcon = (props: IconProps) => (
  <IconBase {...props}>
    <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
    <path d="M7 11V7a5 5 0 0 1 10 0v4" />
  </IconBase>
);

const ClockIcon = (props: IconProps) => (
  <IconBase {...props}>
    <circle cx="12" cy="12" r="10" />
    <polyline points="12 6 12 12 16 14" />
  </IconBase>
);

const ChatIcon = (props: IconProps) => (
  <IconBase {...props}>
    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
  </IconBase>
);

const GoalsIcon = (props: IconProps) => (
  <IconBase {...props}>
    <circle cx="12" cy="12" r="10" />
    <circle cx="12" cy="12" r="6" />
    <circle cx="12" cy="12" r="2" />
  </IconBase>
);

const NotesIcon = (props: IconProps) => (
  <IconBase {...props}>
    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
    <polyline points="14 2 14 8 20 8" />
    <line x1="16" y1="13" x2="8" y2="13" />
    <line x1="16" y1="17" x2="8" y2="17" />
    <polyline points="10 9 9 9 8 9" />
  </IconBase>
);

// ── Person Card ───────────────────────────────────────────────
const PersonCard = ({ name, profile, roleLabel }: PersonCardProps) => {
  const [imgError, setImgError] = useState(false);
  const picture = profile?.profilePicture || "";
  const role = profile?.currentRole || "";
  const company = profile?.company || "";
  const skills = profile?.skills?.slice(0, 3) || [];

  return (
    <div className="flex-1 bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-3">
      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
        {roleLabel}
      </p>
      <div className="flex items-center gap-3">
        {picture && !imgError ? (
          <img
            src={picture}
            alt={name}
            onError={() => setImgError(true)}
            className="w-12 h-12 rounded-[14px] object-cover border-2 border-slate-100 shrink-0"
          />
        ) : (
          <div
            className="w-12 h-12 rounded-[14px] shrink-0 flex items-center justify-center text-white font-bold text-base"
            style={{ background: getGradient(name) }}
            role="img"
            aria-label={name}
          >
            {getInitials(name)}
          </div>
        )}
        <div className="min-w-0">
          <p className="text-sm font-bold text-slate-900 truncate">{name}</p>
          {(role || company) && (
            <p className="text-[11px] text-blue-900 mt-0.5 truncate">
              {role}
              {role && company ? " @ " : ""}
              {company}
            </p>
          )}
        </div>
      </div>
      {skills.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {skills.map((s) => (
            <span
              key={`${name}-${s}`}
              className="px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-[10px] font-semibold text-slate-600"
            >
              {s}
            </span>
          ))}
        </div>
      )}
    </div>
  );
};

// ── Info Row ──────────────────────────────────────────────────
const InfoRow = ({ icon, label, value, accent }: InfoRowProps) => (
  <div className="flex items-start gap-2.5">
    <span className="text-slate-400 mt-0.5 shrink-0">{icon}</span>
    <div className="flex-1">
      <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide">
        {label}
      </p>
      <p
        className={`text-[13px] font-semibold mt-0.5 ${accent ? "" : "text-slate-900"}`}
        style={accent ? { color: accent } : undefined}
      >
        {value}
      </p>
    </div>
  </div>
);

// ── Quick Action Button ───────────────────────────────────────
const QuickAction = ({ icon, label, onClick, color = "#2563eb" }: QuickActionProps) => (
  <button
    onClick={onClick}
    className="flex-1 flex flex-col items-center gap-1.5 py-3.5 px-2.5 rounded-[14px]
      border border-slate-200 bg-white cursor-pointer transition-colors duration-150 hover:bg-slate-50"
  >
    <span style={{ color }}>{icon}</span>
    <span className="text-[11px] font-semibold text-slate-600">{label}</span>
  </button>
);

// ── Main ──────────────────────────────────────────────────────
const SharedHomeTab = ({ slots = [] }: SharedHomeTabProps) => {
  const dispatch = useDispatch<AppDispatch>();
  const [, setSearchParams] = useSearchParams();
  const connect = useSelector(selectConnect);
  const [showReport, setShowReport] = useState(false);
  const [reportDone, setReportDone] = useState(false);

  const onTabChange = useCallback(
    (tab: string) => {
      dispatch(setActiveTab(tab));
      setSearchParams({ tab }, { replace: true });
    },
    [dispatch, setSearchParams],
  );

  if (!connect) return null;

  const {
    mentor,
    mentee,
    mentorProfile,
    menteeProfile,
    confirmedSlot,
    totalAmount,
    paidAt,
    status,
  } = connect;

  const isCompleted = status === "completed";

  return (
    <>
      <div className="flex flex-col gap-6">
        {/* ── Header with Report button at top right ──────── */}
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-[22px] font-bold text-slate-800"> Overview</h1>
            <p className="text-[13px] text-blue-900 mt-1">
              Your active mentorship session details and participants.
            </p>
          </div>
        </div>

        {/* Participants */}
        <div>
          <p className="text-[11px] font-bold text-slate-700 uppercase tracking-widest mb-3">
            Participants
          </p>
          <div className="flex gap-3 flex-wrap">
            <PersonCard
              name={mentor?.name || "Mentor"}
              profile={mentorProfile}
              roleLabel="Mentor"
            />
            <PersonCard
              name={mentee?.name || "Mentee"}
              profile={menteeProfile}
              roleLabel="Mentee"
            />
          </div>
        </div>

        {/* Session Details */}
        <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-4">
          <p className="text-[11px] font-bold text-slate-800 uppercase tracking-widest">
            Session Details
          </p>
          {slots.length > 0
            ? slots.map((slot, i) => (
                <InfoRow
                  key={`${slot?.date || "slot"}-${slot?.startTime || ""}-${slot?.endTime || ""}-${i}`}
                  icon={<CalendarIcon />}
                  label={`Session ${i + 1}${slot.status === "completed" ? " ✓" : ""}`}
                  value={formatSlot(slot)}
                />
              ))
            : confirmedSlot && (
                <InfoRow
                  icon={<CalendarIcon />}
                  label="Confirmed Session"
                  value={formatSlot(confirmedSlot)}
                />
              )}
          {totalAmount != null && (
            <InfoRow
              icon={<TokenIcon />}
              label="Tokens in Escrow"
              value={`${totalAmount} tokens secured`}
              accent="#2563eb"
            />
          )}
          {paidAt && (
            <InfoRow
              icon={<ClockIcon />}
              label="Session Booked On"
              value={formatDate(paidAt)}
            />
          )}
        </div>

        {/* Quick Actions — Report removed from here */}
        <div>
          <p className="text-[11px] font-bold text-slate-800 uppercase tracking-widest mb-3">
            Quick Actions
          </p>
          <div className="flex gap-2.5 flex-wrap">
            <QuickAction
              icon={<ChatIcon size={18} />}
              label="Open Chat"
              onClick={() => onTabChange("chat")}
              color="#2563eb"
            />
            <QuickAction
              icon={<GoalsIcon size={18} />}
              label="Set Goals"
              onClick={() => onTabChange("goals")}
              color="#7c3aed"
            />
            {!isCompleted && (
              <QuickAction
                icon={<NotesIcon size={18} />}
                label="Notes"
                onClick={() => onTabChange("notes")}
                color="#059669"
              />
            )}
          </div>
        </div>
        {/* Report link — bottom of page */}
        <div className="text-center pt-2 pb-1">
          <p className="text-[11px] text-slate-600">
            Something wrong with this session?{" "}
            <button
              type="button"
              onClick={() => setShowReport(true)}
              className="text-red-400 font-semibold hover:text-red-600 hover:underline transition-colors"
            >
              Report an issue
            </button>
          </p>
        </div>
      </div>

      {/* Report Modal */}
      {showReport && !reportDone && (
        <ReportModal
          onClose={() => setShowReport(false)}
          onSuccess={() => {
            setShowReport(false);
            setReportDone(true);
          }}
        />
      )}

      {/* Success Modal */}
      {reportDone && <ReportSuccessModal onBack={() => setReportDone(false)} />}
    </>
  );
};

export default SharedHomeTab;
