...
import { selectConnectId } from "@features/shared-dashboard/models/sharedDashboardSlice";
...
const ChatHeader = ({ name, picture, otherOnline }: ChatHeaderProps) => (
  <div style={{
...
const DateSeparator = memo(({ dateStr }: DateSeparatorProps) => (
...
const ReadReceipt = ({ readAt }: ReadReceiptProps) => (
...
const MessageBubble = memo(({ message, isOwn, otherName, otherPicture }: MessageBubbleProps) => (
  <div style={{
    display: "flex",
...
const TypingIndicator = memo(({ name }: TypingIndicatorProps) => (
...
const LoadMoreButton = ({ onClick, loading }: LoadMoreButtonProps) => (
...
const EmptyState = ({ otherName }: EmptyStateProps) => (
  <div style={{
