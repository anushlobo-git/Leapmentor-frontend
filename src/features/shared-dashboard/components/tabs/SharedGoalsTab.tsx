...
interface Goal extends GoalFields {
  _id: string;
  status?: string;
}
...
interface GoalsState {
  ...
  error: string | null;
  saving: boolean;
  createGoal: (fields: GoalFields) => Promise<{ success?: boolean }>;
  updateGoal: (goalId: string, fields: Partial<GoalFields>) => Promise<{ success?: boolean }>;
  addMilestone: (goalId: string, data: { title: string }) => Promise<{ success?: boolean }>;
  toggleMilestone: (id: string, isCompleted: boolean) => void;
  deleteMilestone: (id: string) => void;
}

interface SharedSessionSlot extends SessionSlot {
  _id?: string;
}

interface SessionsState {
  slots: SharedSessionSlot[];
  loading: boolean;
  savingSlots: Set<number>;
  error: string | null;
  completedSlots: number;
  totalSlots: number;
  progress: number;
  setMeetingLink: (slotIndex: number, link: string) => Promise<{ success?: boolean } | undefined>;
  markSlotComplete: (slotIndex: number) => Promise<{ success?: boolean } | undefined>;
  cancelSlot: (slotIndex: number, reason: string) => Promise<{ success?: boolean } | undefined>;
  rescheduleSlot: (slotIndex: number, newSlot: unknown) => Promise<{ success?: boolean } | undefined>;
}
...
interface GoalCardProps {
  ...
  milestones: Milestone[];
  saving: boolean;
  onAdd: GoalsState["addMilestone"];
  onToggle: GoalsState["toggleMilestone"];
  onDelete: GoalsState["deleteMilestone"];
}
...
const NoGoalState = ({ onSetGoal }: { onSetGoal: () => void }) => (
  <div
...
const SharedGoalsTab = () => {
  ...
  const [feedbackSlotIndex, setFeedbackSlotIndex] = useState<number | null>(null);
  ...
  const {
    ...
    toggleMilestone,
    deleteMilestone,
  } = useGoals(connectRequestId) as GoalsState;
  ...
  const {
    ...
    cancelSlot,
    rescheduleSlot,
  } = useSessions(connectRequestId, onAllComplete) as SessionsState;
  ...
  const handleCreateGoal = async (fields: GoalFields) => {
    const result = await createGoal(fields);
    if (result?.success) setIsEditing(false);
  };

  const handleUpdateGoal = async (goalId: string, fields: Partial<GoalFields>) => {
