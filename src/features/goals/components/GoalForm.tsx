...
interface GoalFormProps {
  ...
  onSave: (goal: { title: string; description: string; startDate: string; endDate: string }) => Promise<void>;
  onCancel: () => void;
  saving: boolean;
}

const GoalForm = ({ initial = {}, onSave, onCancel, saving }: GoalFormProps) => {
  ...
  const handleSave = async () => {
    ...
    if (!startDate || !endDate) {
      setErr("Please set both a start date and an end date");
      return;
    }
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col gap-4">
      ...
      <div>
        <label
          htmlFor="goal-title"
          className="text-xs font-bold text-slate-700 uppercase tracking-wide block mb-1.5"
        >
