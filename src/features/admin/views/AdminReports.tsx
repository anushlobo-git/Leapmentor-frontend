...
import { updateReport, refundReport, deleteReportSession, getReportStats, getReports } from "@features/admin/models/admin.api";
import AdminLayout from "@features/admin/views/AdminLayout";
import StatCard from "@features/admin/views/common/StatCard";

type ReportId = string | number;
type ReportStatus = "open" | "under_review" | "resolved" | "dismissed";
type ReportCategory = "vulgar_chat" | "harassment" | "refund" | "other";
...
const ReportStatCard = StatCard as ComponentType<StatCardProps>;
...
const HandleModal = ({
  ...
  report: Report;
  onClose: () => void;
  onSave: (id: ReportId, updated: Report) => void;
  onRefund: (id: ReportId) => void;
  onDeleteSession: (id: ReportId) => void;
}) => {
  const [selectedStatus, setSelectedStatus] = useState(
    report.status === "resolved" || report.status === "dismissed" ? report.status : "",
  );
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      const res = await updateReport(report.id, { status: selectedStatus, adminNote });
      onSave(report.id, res.data.report);
      onClose();
    } catch (error: unknown) {
      setError((error as ApiError).response?.data?.message || "Failed to update report.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleRefund = async () => {
    try {
      ...
      setError((error as ApiError).response?.data?.message || "Refund failed.");
  ...
  const handleDeleteSession = async () => {
    try {
      setActionLoading(true);
      await deleteReportSession(report.id, adminNote);
      onDeleteSession(report.id);
      onClose();
    } catch (error: unknown) {
      setError((error as ApiError).response?.data?.message || "Failed to delete session.");
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col"
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b shrink-0"
            ...
            <button type="button" aria-label="Close report dialog" onClick={onClose}
              ...
              onMouseEnter={(event: MouseEvent<HTMLButtonElement>) => event.currentTarget.style.background = "#e2e8f0"}
              onMouseLeave={(event: MouseEvent<HTMLButtonElement>) => event.currentTarget.style.background = "#f1f5f9"}>
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                onFocus={(event) => event.currentTarget.style.borderColor = "#93c5fd"}
...
const AdminReports = () => {
  ...
  const [pagination, setPagination] = useState<ReportPagination>({ totalCount: 0, currentPage: 1, totalPages: 1 });
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      setStats(res.data as ReportStats);
  ...
  const fetchReports = useCallback(async (page = 1, q = search, status = statusFilter) => {
    try {
      setLoading(true);
    const params: ReportQuery = { page, limit: 10 };
      if (q) params.search = q;
      if (status) params.status = status;
      const res = await getReports(params);
      setReports((res.data.reports || []) as Report[]);
      setPagination(res.data.pagination as ReportPagination);
  ...
  const handleSearch = (value: string) => {
    ...
    searchTimer.current = setTimeout(() => fetchReports(1, value, statusFilter), 400);
  };
  ...
  const handleSave = (id: ReportId, updated: Report) => {
    setReports((previous) => previous.map((report) => report.id.toString() === id.toString() ? { ...report, ...updated } : report));
    fetchStats();
    showToast("Report updated. Reporter has been notified. ✅");
  };

  const handleRefund = (id: ReportId) => {
    setReports((previous) => previous.map((report) => report.id.toString() === id.toString()
      ? { ...report, status: "resolved", refundProcessed: true } : report));
    fetchStats();
    showToast("Refund processed. Mentee has been notified. ✅");
  };

  const handleDeleteSession = (id: ReportId) => {
    setReports((previous) => previous.map((report) => report.id.toString() === id.toString()
      ? { ...report, status: "resolved", connectRequestId: null } : report));
    fetchStats();
  ...
  if (loading) {
    ...
    tableBody = reports.map((report) => (
      <tr key={report.id} className="transition-colors"
        ...
        onMouseEnter={(event: MouseEvent<HTMLTableRowElement>) => event.currentTarget.style.background = "#fafbfc"}
        onMouseLeave={(event: MouseEvent<HTMLTableRowElement>) => event.currentTarget.style.background = "transparent"}>
        ...
        <td className="px-5 py-4">
          <button type="button" onClick={() => setSelected(report)}
            ...
            onMouseEnter={(event: MouseEvent<HTMLButtonElement>) => { event.currentTarget.style.background = "#2563eb"; event.currentTarget.style.color = "white"; }}
            onMouseLeave={(event: MouseEvent<HTMLButtonElement>) => { event.currentTarget.style.background = "#eff6ff"; event.currentTarget.style.color = "#2563eb"; }}>
            Handle
          </button>
        </td>
      </tr>
    ...
    <AdminLayout>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {STAT_CARDS.map((card) => <ReportStatCard key={card.label} {...card} />)}
        </div>
          ...
          <div className="px-6 py-4 border-b space-y-3" style={{ borderColor: "#e8eaf0" }}>
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="relative">
                ...
                <input value={search} onChange={(event) => handleSearch(event.target.value)}
                  ...
                  onFocus={(event: FocusEvent<HTMLInputElement>) => event.currentTarget.style.borderColor = "#93c5fd"}
                  onBlur={(event: FocusEvent<HTMLInputElement>) => event.currentTarget.style.borderColor = "#e2e8f0"}
                />
              </div>
            </div>
