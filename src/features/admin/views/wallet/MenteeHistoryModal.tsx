...
interface Engagement {
  _id: string;
  mentor?: EngagementParticipant | null;
  mentee?: EngagementParticipant | null;
...
interface MenteeHistoryModalProps {
  mentee: Mentee;
  onClose: () => void;
}

const MenteeHistoryModal = ({
  mentee,
  onClose,
}: MenteeHistoryModalProps) => {
  ...
  useEffect(() => {
    const fetchEngagements = async () => {
      try {
        ...
        const all = (res.data.engagements || []) as Engagement[];
  ...
  const toggleExpand = (engagementId: string) => {
    setExpandedId((previousId) =>
      previousId === engagementId ? null : engagementId,
    );
  };
    ...
    engagementsContent = (
      <div className="space-y-3">
        {engagements.map((eng) => {
          ...
          const mentorName = eng.mentor?.name || "M";
          const mentorAvatar = getAvatarColor(mentorName);
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div
                    ...
                    style={{
                      background: mentorAvatar.bg,
                      color: mentorAvatar.text,
                    }}
                  >
