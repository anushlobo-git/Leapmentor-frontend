...
const UserStatCard = StatCard as ComponentType<StatCardProps>;
const GrowthChart = UserGrowthChart as ComponentType<{ data: GrowthDatum[] }>;
const IndustryChart = MentorIndustryChart as ComponentType<{ data: IndustryDatum[] }>;
...
const AdminUserManagement = () => {
  ...
  const [growthData,  setGrowthData]  = useState<GrowthDatum[]>([]);
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      setStats(res.data as UserStats);
    } catch (error: unknown) {
  ...
  const fetchGrowthData = useCallback(async () => {
    try {
      ...
      setGrowthData((res.data.data || res.data || []) as GrowthDatum[]);
    } catch (error: unknown) {
      logger.error("Failed to fetch growth data", { error });
    }
  }, []);
  ...
  const fetchIndustryData = useCallback(async () => {
    try {
      ...
      setIndustryData((res.data.data || res.data || []) as IndustryDatum[]);
    } catch (error: unknown) {
      logger.error("Failed to fetch industry data", { error });
    }
  }, []);
  ...
  const fetchUsers = useCallback(async (page = 1, q = search, role = roleFilter, blocked = showBlocked) => {
    try {
      setLoading(true);
      const params: UserQuery = { page, limit: 15 };
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimer.current = setTimeout(() => fetchUsers(1, value, roleFilter, showBlocked), 400);
  };
  ...
  const handleBlockedToggle = (isBlocked: boolean) => {
