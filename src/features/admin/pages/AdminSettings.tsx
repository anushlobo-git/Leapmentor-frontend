...
const FormField = <T extends FieldValues,>({
  ...
  <div>
    ...
    <input
      ...
      onFocus={(event: FocusEvent<HTMLInputElement>) => (event.currentTarget.style.borderColor = focusColor)}
...
const AdminSettings = () => {
  ...
  const onCommissionSubmit = async (data: CommissionForm) => {
    try {
      setSavingCommission(true);
      await updateCommissionSettings(data.commission);
      showToast(`Commission rate set to ${data.commission}%`);
    } catch (error: unknown) {
      showToast(
        (error as ApiError).response?.data?.message || "Failed to update commission.",
        "error",
      );
    } finally {
  ...
  const onAdminSubmit = async (data: AddAdminForm) => {
    try {
      setTempPw("");
      ...
      showToast(
        (error as ApiError).response?.data?.message || "Failed to create admin.",
        "error",
      );
    }
  };
