...
import { getCommissionSettings, updateCommissionSettings, addAdmin } from "@features/admin/api/admin.api";
...
const AdminSettings = () => {
  ...
  const onCommissionSubmit = async (data) => {
    try {
      ...
      await updateCommissionSettings(parseFloat(data.commission));
  ...
  const onAdminSubmit = async (data) => {
    try {
      setTempPw("");
      const res = await addAdmin({ name: data.name.trim(), email: data.email.trim() });
