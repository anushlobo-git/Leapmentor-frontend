...
interface CustomTooltipProps {
  active?: boolean;
  payload?: Array<{ payload?: IndustryDatum & { pct: string } }>;
}
...
const CustomXTick = ({ x = 0, y = 0, payload }: CustomXTickProps) => {
  return (
...
const CustomBarShape = (props: Partial<BarShapeProps>) => {
  const { index } = props;
  return (
    <Rectangle
      {...props}
...
// ══════════════════════════════════════════════════════════════
const MentorIndustryChart = ({ data = [] }: { data?: IndustryDatum[] }) => {
  if (!data.length) {
    return (
      <div
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        {/* Legend: Updated safely to max-w-55 */}
        <div className="flex flex-wrap gap-x-3 gap-y-1.5 max-w-55 justify-end">
          {chartData.map((item) => (
            <div
              key={`legend-${item.industry}`}
              className="flex items-center gap-1.5"
            >
              <div
                className="w-2 h-2 rounded-full shrink-0"
                style={{
                  background: PALETTE[chartData.indexOf(item) % PALETTE.length],
                }}
              />
      ...
      <ResponsiveContainer width="100%" height={260}>
        ...
        >
          ...
          {/* Passed the newly extracted component explicitly as a JSX node layout declaration */}
