...
// src/components/admin/common/UserGrowthChart.tsx
...
// Always returns the same shape ({ line, xs, ys }) regardless of input,
// so callers never have to branch on "did this return a string or an object".
const smoothPath = (
  points: number[],
  width: number,
  height: number,
  ...
  const ys = points.map((value) => height - ((value - minimum) / range) * (height * 0.8) - height * 0.1);
...
const UserGrowthChart = ({ data = [] }: UserGrowthChartProps) => {
  ...
  const { line, xs, ys } = smoothPath(values, CHART_W, H, scaleMin, scaleMax);
      ...
      <div className="relative" style={{ overflowX: "auto" }}>
        ...
        >
          ...
          <g transform={`translate(${LABEL_W}, 0)`}>
            ...
            {xs.map((x, i) => {
              const pointKey = `${sliced[i]?.label ?? "point"}-${x}`;
              return (
                <g key={pointKey}>
                  <rect
                    x={x - 10}
