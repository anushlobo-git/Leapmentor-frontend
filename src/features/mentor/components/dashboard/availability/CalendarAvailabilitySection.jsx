...
const addSlotToDate = (dateStr) => (prev) =>
  prev.map((d) =>
    d.date === dateStr
...
const removeSlotFromDate = (dateStr, index) => (prev) =>
  prev.map((d) =>
    d.date === dateStr
...
const updateSlotInDate = (dateStr, index, field, value) => (prev) =>
  prev.map((d) =>
    d.date === dateStr
      ? {
          ...d,
...
const CalendarAvailabilitySection = ({
  ...
  const handleAddSlot = (dateStr) => setSpecificDates(addSlotToDate(dateStr));
...
CalendarGrid.propTypes = {
  ...
  busySlots: PropTypes.arrayOf(PropTypes.shape({ start: PropTypes.string })),
};
