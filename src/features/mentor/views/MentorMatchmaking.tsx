...
const mentorsData: MatchmakingMentor[] = [
...
const MentorMatchmaking = () => {
  ...
  const filteredMentors = mentorsData.filter((m: MatchmakingMentor) => {
    const keywordMatch = filters.keyword
      ? !(
          m.name.toLowerCase().includes(filters.keyword.toLowerCase()) ||
          m.skills.some((s: string) => s.toLowerCase().includes(filters.keyword.toLowerCase())) ||
  ...
  const handleBookSession = (mentor: MatchmakingMentor) => {
  ...
  return (
    <div className="min-h-screen px-4 py-10 bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredMentors.length ? filteredMentors.map((mentor: MatchmakingMentor) => (
          <div
            key={mentor.name}
