...
import {
  useCalendarAvailabilityPresenter,
  getTodayLocal,
  getSlotError,
  timeToMins,
  type DateEntry,
  ...
  type BusySlot,
  type CalendarEvent,
} from "@features/mentor/presenters/useCalendarAvailabilityPresenter";
...
const getOverlappingBusy = (dateStr: string, slot: Slot, busySlots: BusySlot[] | undefined) => {
...
const getEventsForDate = (dateStr: string, events: CalendarEvent[] | undefined) => {
...
const parse24 = (timeStr: string | undefined) => {
...
const format24 = ({ hour12, minute, period }: { hour12: number; minute: number; period: string }) => {
  let h = hour12 % 12;
  if (period === "PM") h += 12;
...
const TimePicker = ({ value, onChange, hasError = false }: { value?: string; onChange: (val: string) => void; hasError?: boolean }) => {
  ...
  const commitParts = (newH: number, newM: number, newP: string) => {
  ...
  // S3358 Fixed: Extracted nested ternary operation into procedural conditions
  const getWrapperClasses = () => {
    if (hasError) return "border-red-400 ring-2 ring-red-100";
    if (isEditing) return "border-blue-400 ring-2 ring-blue-100";
  ...
  return (
    <>
      <div
        ref={wrapperRef}
        className={`flex items-center bg-white border rounded-lg transition-all duration-150 ${getWrapperClasses()}`}
        style={{ width: "120px" }}
      >
        <input
          ref={inputRef}
...
// ─── EventTooltip ─────────────────────────────────────────────────────────────
const EventTooltip = ({ events, isBusyOnly }: { events: CalendarEvent[]; isBusyOnly: boolean }) => (
    ...
    {isBusyOnly ? (
      ...
      <div className="space-y-2">
        {events.map((e, i: number) => (
          // S6479 Fixed: Appended explicit identifying strings to prevent primitive mapping conflicts
          <div
            key={`tooltip-event-${e.summary}-${i}`}
  ...
  return (
    <div>
      ...
      <div style={{ ...GRID_7, gap: "3px", overflow: "visible" }}>
        {cells.map((day, idx: number) => {
          // S6479 Fixed: Composed descriptive stable values to separate regular loops from indices
          if (!day) return <div key={`empty-cell-${year}-${month}-${idx}`} />;
          ...
          // S3358 Fixed: Extracted nested ternary blocks into flattened procedural class selectors
          const getCellClassName = () => {
            if (isPast) return "text-slate-200 cursor-not-allowed";
            if (isSelected) return "bg-blue-900 text-white shadow-sm scale-105";
            if (isToday)
              return "bg-blue-50 text-blue-900 ring-1 ring-blue-300 font-bold hover:bg-blue-100";
            return "text-slate-600 hover:bg-slate-100 hover:text-slate-800";
          };

          const getIndicatorColor = () => {
            if (isPast) return "bg-orange-200";
            if (isSelected) return "bg-yellow-300";
            return "bg-orange-400";
          };

          return (
            <div key={`active-day-${dateStr}`} className="relative">
              <button
                type="button"
                disabled={isPast}
                ...
                className={`relative rounded-lg text-[11px] font-semibold flex flex-col items-center justify-center transition-all duration-150 ${getCellClassName()}`}
              >
                {day}
                <div className="absolute bottom-0.5 left-1/2 -translate-x-1/2 flex gap-0.5">
                  ...
                  {hasIndicator && (
                    <span
                      className={`w-1 h-1 rounded-full ${getIndicatorColor()}`}
                    />
                  )}
                </div>
              </button>
...
// ─── BusyBadge ────────────────────────────────────────────────────────────────
const BusyBadge = ({ overlaps }: { overlaps?: BusySlot[] }) => {
  if (!overlaps?.length) return null;
  return (
    <div className="flex flex-col gap-1">
      {overlaps.map((b, i: number) => (
        // S6479 Fixed: Swapped plain indices for detailed composite string bounds
        <span
          key={`overlap-badge-${b.start}-${b.end}-${i}`}
...
interface DateSlotEditorProps {
  ...
  onRemoveSlot: (dateStr: string, index: number) => void;
  onUpdateSlot: (dateStr: string, index: number, field: string, value: string) => void;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-3.5 py-2.5 space-y-3">
        {dateEntry.slots.map((slot, index: number) => {
          ...
          return (
            // S6479 Fixed: Swapped bare index configurations for precise structural strings
            <div
              key={`slot-row-${dateEntry.date}-${slot.startTime}-${index}`}
...
const CalendarAvailabilitySection = (props: CalendarAvailabilitySectionProps) => {
  const { specificDates, setSpecificDates, googleCalendarConnected } = props;
  const {
    calYear,
    calMonth,
    busySlots,
    ...
    handlePrevMonth,
    handleNextMonth,
  } = useCalendarAvailabilityPresenter(props);
        ...
        <div className="flex-1 min-w-0">
          ...
          {futureDates.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              >
                {futureDates.map((dateEntry: DateEntry) => (
                  <DateSlotEditor
                    key={`slot-editor-${dateEntry.date}`}
                    dateEntry={dateEntry}
