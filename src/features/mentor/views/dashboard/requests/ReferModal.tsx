...
import {
  useReferModalPresenter,
  getInitials,
  type SimilarMentor,
  type ReferModalRequest,
} from "@features/mentor/presenters/useReferModalPresenter";
...
const ReferModal = ({ request, onClose, onReferred }: ReferModalProps) => {
  const {
    mentors,
    mySkills,
    loading,
    ...
    mentorListContent = (
      <>
        ...
        <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
          {mentors.map((mentor: SimilarMentor) => {
            const mInitials = getInitials(mentor.user?.name);
            const isSelected = selected?._id === mentor._id;
            const matchingSkills = mentor.skills.filter((s: string) =>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {matchingSkills.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {matchingSkills.slice(0, 4).map((skill: string) => (
                          <span
                            key={skill}
